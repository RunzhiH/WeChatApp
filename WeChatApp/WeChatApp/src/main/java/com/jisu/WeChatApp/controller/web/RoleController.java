...
public class RoleController {
	...
	public ModelAndView toPage(Permission permission) {
		...
		try {
			if (null != permission) {
				...
				roleServiceImpl.addPermission(permission);
	...
	public ModelAndView permList() {
		...
		try {
			List<Permission> permList = roleServiceImpl.permList();
		...
		try {
			if (null != permission) {
				...
				if (0 == type) {
					...
					// 编辑权限
					roleServiceImpl.updatePerm(permission);
				} else if (1 == type) {
	...
	@ResponseBody
	public Permission getPerm(@RequestParam("id") String id) {
		logger.debug("获取权限--id-" + id);
		try {
			if (StringUtils.isNotBlank(id)) {
				Permission perm = roleServiceImpl.getPermission(id);
	...
	public String del(@RequestParam("id") String id) {
		...
		try {
			if (StringUtils.isNotBlank(id)) {
				return roleServiceImpl.delPermission(id);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Role> getRoleList() {
		...
		try {
			roleList = roleServiceImpl.roleList();
	...
	public String addRole(@RequestParam("permIds") String permIds, Role role) {
		...
		try {
			...
			return roleServiceImpl.addRole(role, permIds);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ModelAndView updateRole(@PathVariable("id") String id) {
		...
		try {
			...
			Role rvo = roleServiceImpl.findRoleAndPerms(id);
			// 角色下的权限
			List<RolePermission> rpks = rvo.getRolePerms();
			// 获取全部权限数据
			List<Permission> pvos = roleServiceImpl.findPerms();
	...
	public String setRole(@RequestParam("rolePermIds") String permIds, Role role) {
		...
		try {
			...
			return roleServiceImpl.updateRole(role, permIds);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String delRole(@RequestParam("id") String id) {
		...
		try {
			if (StringUtils.isNotBlank(id)) {
				return roleServiceImpl.delRole(id);
			}
		} catch (Exception e) {
			e.printStackTrace();
