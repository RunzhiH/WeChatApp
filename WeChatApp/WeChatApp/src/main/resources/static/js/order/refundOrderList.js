$(function() {
	layui.use('table', function() {
		...
		tableIns = table.render({
			...
			url : '/web/order/getRefundOrderList',
		...
		// 监听工具条
		table.on('tool(refundOrderTable)', function(obj) {
			var data = obj.data;
			if (obj.event === 'datil') {
	...
	layui.use([ 'form', 'laydate' ], function() {
		...
		form.on('submit(searchOrderSubmit)', function(obj) {
			// 重新加载table
			tableIns.reload('refundOrderList', {
				where : obj.field,
				page : {
					curr : pageCurr
				// 从当前页码开始
...
function getOrderDatil(id) {
	if (id != null) {
		window.location.href = "/web/order/refundOrderDatil/" + id + "?callback=" + getCallback();
	} else {
