        ...
        $.get("/web/rotation/getRotationList",function(data){
            if(data!=null){
                ...
                $.each(data, function (index, item) {
                    var td=$("<tr><td><img src='"+item.rotationUrl+"'/></td>"
                    +"<td>"+item.toPage+"</td>"
                    +"<td>"+item.rotationDesc+"</td>"
                    +"<td>"+DateUtils.formatDate(item.createTime)+"</td>"
                    +"<td>"
                    +"<button class=\"layui-btn layui-btn-xs\" onclick=\"update('"+item.rotationId+"',0)\">编辑</button>"
                        +"<button class=\"layui-btn layui-btn-danger layui-btn-xs\" onclick=\"del(\'"+item.rotationId+"\')\">删除</button></td></tr>");
                    tbody.append(td);
                });
            }
        });
...
function update(id,type){
    if(null!=id){
    	$("#type").val(type);
        $("#rotationId").val(id);
        $.get("/web/rotation/getRotation",{"rotation_id":id},function(data) {
            // console.log(data);
            if(null!=data){
...
function del(id){
    ...
    if(null!=id){
        layer.confirm('您确定要删除该服务类目吗？', {
            ...
            $.post("/web/rotation/delRotation",{"rotation_id":id},function(data){
                if(data=="ok"){
                    //回调弹框
